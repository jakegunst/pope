const PeruLevel = {
  name: 'Peru',
  width: 400,
  height: 180,
  tileSize: 32, // or whatever your game uses
  data: [
    // ... your level data array from the converter
const PeruLevelData = [
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                          K                    
",
  "                                                                                                                                                                                                                                                                                                                                                                                          D                 E  
",
  "                                                                                                                                                                                                                                                                                                                                                                                                            E  
",
  "                                                                                                                                                                                                                                                                                                                                                                                 C  TTTT                GGGGGGGG
",
  "                                                                                                                                                                                                                                                                                                                                                                                                       G       
",
  "                                                                                                                                                                                                                                                                                                                                                                                                      G        
",
  "                                                                                                                                                                                                                                                                                                                                                                              C  WALKER                   G         
",
  "                                                                                                                                                                                                                                                                                                                                                                                 P            K PPPPP          
",
  "                                                                                                                                                                                                                                                                                                                                                                                P             M                
",
  "                                                                                                                                                                                                                                                                                                                                                                           C   P                               
",
  "                                                                                                                                                                                                                                                                                                                                                                    FLYER        WALKERP                                
",
  "                                                                                                                                                                                                                                                                                                                                                                             P                                 
",
  "                                                                                                                                                                                                                                                                                                                                                                        C   P                                  
",
  "                                                                                                                                                                                                                                                                                                                                                                           P                                   
",
  "                                                                                                                                                                                                                                                                                                                                                                          P                                    
",
  "                                                                                                                                                                                                                                                                                                                                                                         P                                     
",
  "                                                                                                                                                                                                                                                                                                                                                                    C  SP                                      
",
  "                                                                                                                                                                                                                                                                                                                                                                    GGGG                                       
",
  "                                                                                                                                                                                                                                                                                                                                                                    G                                          
",
  "                                                                                                                                                                                                                                                                                                                                                                   SG                                          
",
  "                                                                                                                                                                                                                                                                                                                                                          FLYER      GGGG                                          
",
  "                                                                                                                                                                                                                                                                                                                                                                 G                                             
",
  "                                                                                                                                                                                                                                                                            FLYER                                                                       KKKK      C SG                                             
",
  "                                                                                                                                                                                                                                                                                                                                                    DDDD      GGGG                                             
",
  "                                                                                                                                                                                                                                                          C           C           C                                                                S          G                                                
",
  "                                                                                                                                                                                                                                                                                              WALKER                                                   S          SG                                                
",
  "                                                                                                                                                                                                                                                        DDDD                    DDDD                                                             S         GGGG                                                
",
  "                                                                                                                                                                                                                                                                                        WALKER                                                       S     C    G                                                   
",
  "                                                                                                                                                                                                                                       PPPPPP                 KKKK        KKKK        KKKK                                                     S          SG                                                   
",
  "                                                                                                                                                                                                             L                                                                                                                                S         GGGG                                                   
",
  "                                                                                                                                                                                                                                                MMMM                MMMM                    MMMM                                             S                                                                 
",
  "                                                                                                                                                                                                                                      KK                                                             TTTTTTT                         C      S                                                                  
",
  "                                                                                                                                                                                                           TTTTT                     KK                                                                                                    S                                                                   
",
  "                                                                                                                                                                                                                                 PPPPPP                                                                                                 WALKER S                                                                    
",
  "                                                                                                                                                                                                                                                                                                                                C     PPPP                                                                     
",
  "                                                                                                                                                                                                                                                                                                                WALKER                    P                                                                         
",
  "                                                                                                                                                                                                                                 KK                                                                           TTTT               PPPP                                                                          
",
  "                                                                                                                                                                                                                                KK                                                                                            WALKER P                                                                              
",
  "                                                                                                                                                                                                     TTTTT      TTTTT       PPPPP                                                                                     C     PPPP                                                                               
",
  "                                                                                                                                                                                                                                                                                                      TTTT                 P                                                                                   
",
  "                                                                                                                                                        C                                                                                                                                                              PPPP                                                                                    
",
  "                                                                                                                                                                                                                           KK                                                                                       WALKER P                                                                                        
",
  "                                                                                                                                                                                                                          KK                                                                                  C   PPPP                                                                                         
",
  "                                                                                                                                                                                              TTTTTTTTTT          TTTTTTTTTT                                                                                     P                                                                                             
",
  "                                                                                                                                                 TTTTT                                                                                                                                            TTTTTTTTTTPPPPP                                                                                              
",
  "                                                                                                                                                                                       KKK                KKKKKK                                                                                                                                                                                               
",
  "                                                                                                                                                                   C                   P P                                                                                                                                                                                                                     
",
  "                                                                                                                                                                               KKKKK   P P                                                                                                                                                                                                                     
",
  "                                                                                                                                                                        KKKK   P   P   P P    WALKER    WALKER                                                                                                                                                                                                           
",
  "                                                                                                                                            TTTTT     KKKKK     PPPPPPPPPPPPPPP     PPP   PPPPPPPPPPPPPP                                                                                                                                                                                                       
",
  "                                                                                                                                         C                                                                                                                                                                                                                                                                     
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                       TTTTT                                                                                                                                                                                                                                                                   
",
  "                                                                                                                                C                                                                                                                                                                                                                                                                              
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                              TTTTTTTT                                                                                                                                                                                                                                                                         
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                  FLYER                                                                                                                                                                                                                                                            
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                      C                                                                                                                                                                                                                                                                        
",
  "                                                                                                                              MMMMPPPPPPPP                                                                                                                                                                                                                                                                     
",
  "                                                                                                                                          P                                                                                                                                                                                                                                                                    
",
  "                                                                                                                                           P                                                                                                                                                                                                                                                                   
",
  "                                                                                                                                           P                                                                                                                                                                                                                                                                   
",
  "                                                                                                                                           P                                                                                                                                                                                                                                                                   
",
  "                                                                                                                                           P   WALKER      WALKER                                                                                                                                                                                                                                                        
",
  "                                                                                                                                            TTTTTTTTTTTTTTT                                                                                                                                                                                                                                                    
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                           TTTTT                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                  C                                                                                                                                                                                                                                            
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                TTTTT                                                                                                                                                                                                                                          
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                     TTTTT                                                                                                                                                                                                                                     
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                   TTTT                                                                                                                                                                                                                                        
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                TTTTT                                                                                                                                                                                                                                          
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                         WALKER  WALKER                                                                                                                                                                                                                                                  
",
  "                                                                                                                                                      TTTTTTTTTT                                                                                                                                                                                                                                               
",
  "                                                                                                                                         C         C                                                                                                                                                                                                                                                           
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                       TTTTT     TTTTT                                                                                                                                                                                                                                                         
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                            FLYER                                                                                                                                                                                                                                                                                  
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                  TTTTT     TTTTT                                                                                                                                                                                                                                                              
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                                                                                                                                                                                                                                                                                                               
",
  "                                                                                                                        GGGGGTTTTTTTTTTTTTTT                                                                                                                                                                                                                                                                   
",
  "                                                                                                                       G                                                                                                                                                                                                                                                                                       
",
  "                                                                                            C                         G                                                                                                                                                                                                                                                                                        
",
  "                                                                                                                     G                                                                                                                                                                                                                                                                                         
",
  "                                                                                                                    G                                                                                                                                                                                                                                                                                          
",
  "                                                                                                               WALKER   G                                                                                                                                                                                                                                                                                           
",
  "                                                                                                PPPPMMMM    GGGGGGG                                                                                                                                                                                                                                                                                            
",
  "                                                                                               P                                                                                                                                                                                                                                                                                                               
",
  "                                                                                            C  P                                                                                                                                                                                                                                                                                                               
",
  "                                                                                               P                                                                                                                                                                                                                                                                                                               
",
  "                                                                                               P                                                                                                                                                                                                                                                                                                               
",
  "                                                                                           PPPP                                                                                                                                                                                                                                                                                                                
",
  "                                                                                          P                                                                                                                                                                                                                                                                                                                    
",
  "                                                                                          P                                                                                                                                                                                                                                                                                                                    
",
  "                                                                                          P                                                                                                                                                                                                                                                                                                                    
",
  "                                                                                          P                                                                                                                                                                                                                                                                                                                    
",
  "                                                                                      PPPP                                                                                                                                                                                                                                                                                                                     
",
  "                                                                                  C  P                                                                                                                                                                                                                                                                                                                         
",
  "                                                                                     P                                                                                                                                                                                                                                                                                                                         
",
  "                                                                                     P                                                                                                                                                                                                                                                                                                                         
",
  "                                                              FLYER      FLYER          PPPPP                                                                                                                                                                                                                                                                                                                          
",
  "                                                                            GGGG                                                                                                                                                                                                                                                                                                                               
",
  "                                                                           G                                                                                                                                                                                                                                                                                                                                   
",
  "                                                                          G                                                                                                                                                                                                                                                                                                                                    
",
  "                                                                         G                                                                                                                                                                                                                                                                                                                                     
",
  "                                                                        G                                                                                                                                                                                                                                                                                                                                      
",
  "                                                                       G                                                                                                                                                                                                                                                                                                                                       
",
  "                                          C                           G                                                                                                                                                                                                                                                                                                                                        
",
  "                                                            GGGGGGGGGG                                                                                                                                                                                                                                                                                                                                         
",
  "                                                           G                                                                                                                                                                                                                                                                                                                                                   
",
  "                                                  KKK     G                                                                                                                                                                                                                                                                                                                                                    
",
  "                                  GGGGGG     GGGGGGGGGGGGG                                                                                                                                                                                                                                                                                                                                                     
",
  "                                 G                                                                                                                                                                                                                                                                                                                                                                             
",
  "                                G                                                                                                                                                                                                                                                                                                                                                                              
",
  "      C      C                 G                                                                                                                                                                                                                                                                                                                                                                               
",
  "                         WALKER    G                                                                                                                                                                                                                                                                                                                                                                                
",
  "                    GGGGGGGGGG                                                                                                                                                                                                                                                                                                                                                                                 
",
  "                   G                                                                                                                                                                                                                                                                                                                                                                                           
",
  " X                G                                                                                                                                                                                                                                                                                                                                                                                            
",
  " XX               G                                                                                                                                                                                                                                                                                                                                                                                             
",
  "",
];

// Level metadata
const PeruLevelMeta = {
  width: 410,
  height: 180,
  playerStart: { x: 2, y: 178 }
};

  ],
  playerStart: { x: 1 * 32, y: 177 * 32 },
  enemies: {
    walkers: [], // populated during parsing
    flyers: []   // populated during parsing
  }
};
